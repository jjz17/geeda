...
@pytest.mark.parametrize(
    ...
    [
        ...
        [pd.DataFrame({"a": [1.2, 1.2, 1.2, 1.3, 1.3, 1.3]}), "a", 0.4, True],
        ...
        [pd.DataFrame({"a": [1.1, 1.11, 1.12, 1.13, 1.31, 1.32]}), "a", 0.4, False],
    ],
)
def test_is_categorical(df, column, upper_threshold, expected_classification):
    """
    Tests that the function is_categorical works as expected.
    """

    # Assert
    assert is_categorical(df, column, upper_threshold) == expected_classification
...
@pytest.mark.parametrize(
    ...
    [
        ...
        [
            pd.DataFrame({"a": [1, 1, 1, 2, 2, None, None, None, None, None]}),
            "a",
            0.5,
            True,
            True,
...
def test_dropna_is_categorical(
    df, column, upper_threshold, dropna, expected_classification
):
    """
    Tests that the function is_categorical works as expected.
    ...
    # Assert
    assert (
        is_categorical(df, column, upper_threshold, dropna) == expected_classification
    )
...
def test_raise_exceptions(
    ...
    """
    Tests that the function is_categorical raises exceptions as expected.
    """

    with pytest.raises(expected_exception, match=expected_message):
        is_categorical(df, column, upper_threshold)
